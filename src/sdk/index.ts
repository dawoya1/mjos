...
export class MJOSSDK {
  ...
  constructor(options: SDKOptions = {}) {
    ...
    this.logger = new Logger('MJOSSDK');
  ...
  // Memory Operations
  async storeMemory(content: any, tags: string[] = [], importance: number = 0.5, type?: string): Promise<string> {
    if (this.mjos) {
      return this.mjos.getMemorySystem().store(content, tags, importance, type as any);
    }
  ...
  async addKnowledge(knowledgeData: any): Promise<string> {
    if (this.mjos) {
  ...
  async reason(type: string, input: any, context: any = {}): Promise<any> {
    if (this.mjos) {
  ...
  // Agent Operations
  async createAgent(definition: any): Promise<string> {
    if (this.mjos) {
  ...
  async assignTaskToAgent(taskId: string, agentId?: string): Promise<string> {
    if (this.mjos) {
  ...
  async sendMessage(from: string, to: string, content: any, type: string = 'notification'): Promise<string> {
    if (this.mjos) {
  ...
  async createChannel(name: string, participants: string[], type: string = 'multicast'): Promise<string> {
    if (this.mjos) {
  ...
  private generateTypeScriptCode(includeTypes: boolean, includeExamples: boolean): string {
...
`;
  ...
  private generateJavaScriptCode(includeExamples: boolean): string {
...
`;
  ...
  private generatePythonCode(includeExamples: boolean): string {
...
`;
  ...
  private generateJavaCode(includeExamples: boolean): string {
...
`;
  ...
  private generateCSharpCode(includeExamples: boolean): string {
    let code = `// MJOS C# SDK
using System;
...
`;
  ...
  private generateMarkdownDocs(includeExamples: boolean, includeAPI: boolean): string {
...
`;
  ...
  private generateHTMLDocs(includeExamples: boolean, includeAPI: boolean): string {
    return `<!DOCTYPE html>
<html>
<head>

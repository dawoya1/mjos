...
export class MoxiaoyiMemoryManager {
  private mjos: MJOS;
  private memoryIndex: Map<string, MemoryRecord> = new Map();
  ...
  async recallMemories(query: {
    ...
    if (query.content) {
      results = results.filter(memory => 
        memory.content.toLowerCase().includes(query.content!.toLowerCase())
      );
    }
  ...
  async generateMeetingMinutes(meetingData: {
    ...
    discussions: string[];
    decisions: string[];
    actionItems: Array<{ task: string; assignee: string; deadline: Date }>;
   ...
   * 生成档案报告
   */
  async generateArchiveReport(period: { start: Date; end: Date }): Promise<ArchiveReport> {
   ...
   * 智能标签提取
   */
  private extractSmartTags(content: string): string[] {
    const tags: string[] = [];
   ...
   * 查找相关记忆
   */
  private findRelatedMemories(content: string, tags: string[]): string[] {
   ...
   * 更新索引
   */
  private updateIndexes(memory: MemoryRecord): void {
    // 更新标签索引
   ...
   * 生成记忆ID
   */
  private generateMemoryId(category: string, projectId?: string): string {
  ...
  // 辅助方法
  private extractAgenda(discussions: string[]): string[] {
    return discussions.map((d, i) => `议题${i + 1}: ${d.substring(0, 50)}...`);
  }

  private generateMeetingSummary(meetingData: any): string {
  ...
  private extractKeyInsights(memories: MemoryRecord[]): string[] {
    // 简化的洞察提取
  ...
  private generateRecommendations(memories: MemoryRecord[]): string[] {
    return [
      '建议定期整理和归档重要记忆',
      '加强项目相关记忆的关联性',
      '提高记忆标签的标准化程度'

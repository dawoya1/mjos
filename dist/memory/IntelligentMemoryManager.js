...
exports.IntelligentMemoryManager = exports.ThinkingMethod = exports.MemoryType = void 0;
...
(function (MemoryType) {
    ...
    MemoryType["WORKING"] = "working"; // 工作记忆
...
(function (ThinkingMethod) {
    ThinkingMethod["MCKINSEY_7_STEPS"] = "mckinsey_7_steps";
    ThinkingMethod["GLOBAL_OVERVIEW"] = "global_overview";
    ThinkingMethod["CONTRADICTION_ANALYSIS"] = "contradiction_analysis";
})(ThinkingMethod || (exports.ThinkingMethod = ThinkingMethod = {}));
/**
 * 智能记忆管理器
 * 实现记忆系统分层策略和自动调用机制
 */
class IntelligentMemoryManager extends events_1.EventEmitter {
    constructor() {
        super();
        this.autoRetrievalEnabled = true;
        this.logger = new core_1.Logger('IntelligentMemoryManager');
        this.longTermMemory = new index_1.MemorySystem();
        this.workingMemory = new index_1.MemorySystem();
    ...
    initializeClassificationRules() {
        return [
            // 长期记忆规则
            {
                ...
                condition: (content, tags) => content.length > 100 ||
            ...
            },
            {
                ...
                condition: (content, tags) => /code|config|architecture|技术|代码|配置|架构/.test(content) ||
            ...
            },
            {
                ...
                condition: (content, tags) => /learn|knowledge|best practice|教训|学习|知识|最佳实践/.test(content) ||
            ...
            },
            {
                ...
                condition: (content, tags) => /preference|workflow|personal|偏好|工作流程|个人/.test(content) ||
            ...
            // 工作记忆规则
            {
                ...
                condition: (content, tags) => Boolean(tags && tags.some(tag => ['context', 'conversation', 'current'].includes(tag.toLowerCase()))),
            ...
            },
            {
                ...
                condition: (content, tags) => /role|activation|mode|角色|激活|模式/.test(content) ||
            ...
            },
            {
                ...
                condition: (content, tags) => /reasoning|process|temporary|推理|过程|临时/.test(content) ||
     ...
     * 根据分层策略自动分类和存储
     */
    async smartStore(content, options) {
        ...
        // 存储记忆
        const memoryId = memorySystem.store(content, [...tags, memoryType, classification.name], finalImportance);
     ...
     * 分类记忆
     */
    classifyMemory(content, tags) {
        // 找到匹配的规则
        const matchedRule = this.classificationRules.find(rule => rule.condition(content, tags));
        if (matchedRule) {
            return {
     ...
     * 根据查询内容智能检索相关信息
     */
    async autoRetrieve(query, context) {
     ...
     * 计算相关性得分
     */
    calculateRelevanceScore(memory, queryKeywords) {
        if (queryKeywords.length === 0)
            return 0;
        const memoryText = (memory.content + ' ' + (memory.tags || []).join(' ')).toLowerCase();
        const matchCount = queryKeywords.filter(keyword => memoryText.includes(keyword)).length;
     ...
     */
    async deepThink(problem, method = ThinkingMethod.MCKINSEY_7_STEPS) {
     ...
     * 应用麦肯锡七步法
     */
    applyMcKinsey7Steps(problem, memories) {
        const steps = [
            '1. 定义问题',
            '2. 拆解问题',
            '3. 筛选关键问题',
     ...
     * 应用纵览全局法
     */
    applyGlobalOverview(problem, memories) {
     ...
     * 应用矛盾分析法
     */
    applyContradictionAnalysis(problem, memories) {
     ...
     * 定期清理过期的工作记忆
     */
    async cleanupWorkingMemory(maxAge = 24 * 60 * 60 * 1000) {
     ...
     * 启用/禁用自动检索
     */
    setAutoRetrievalEnabled(enabled) {
...
exports.IntelligentMemoryManager = IntelligentMemoryManager;
//# sourceMappingURL=IntelligentMemoryManager.js.map
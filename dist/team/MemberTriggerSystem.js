...
exports.MemberTriggerSystem = void 0;
class MemberTriggerSystem {
    constructor() {
     ...
     * 获取成员的专业能力评分
     */
    getMemberSkillScore(memberId, requiredSkills) {
        const member = this.members.get(memberId);
        if (!member)
            return 0;
        const memberSkills = [...member.skills, ...member.specialties].map(s => s.toLowerCase());
        const matchedSkills = requiredSkills.filter(skill => memberSkills.some(memberSkill => memberSkill.includes(skill.toLowerCase()) ||
     ...
     * 推荐最适合的团队成员
     */
    recommendMember(projectDescription, requiredSkills = []) {
     ...
     * 生成团队成员介绍
     */
    generateMemberIntroduction(memberId) {
        const member = this.members.get(memberId);
        if (!member)
            return '未找到该团队成员';
        return `
...
exports.MemberTriggerSystem = MemberTriggerSystem;
exports.default = MemberTriggerSystem;
//# sourceMappingURL=MemberTriggerSystem.js.map